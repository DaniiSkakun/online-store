...
async function createAdmin() {
    try {
        ...
        const response = await axios.post(`${API_BASE}/user/registration`, {
        ...
        console.error('❌ Ошибка создания админа:', error.response?.data?.message);
        throw error;
    }
}
...
async function createCities(token) {
    ...
    for (const city of cities) {
        try {
            const response = await axios.post(`${API_BASE}/city`, city, {
                headers: { Authorization: `Bearer ${token}` }
            });
            cityIds[city.name] = response.data.id;
            console.log(`✅ Создан город: ${city.name} (ID: ${response.data.id})`);
        } catch (error) {
            console.error(`❌ Ошибка создания города ${city.name}:`, error.response?.data?.message);
        }
    }
...
async function createDistricts(token, cityIds) {
    ...
    for (const district of districts) {
        try {
            await axios.post(`${API_BASE}/district`, district, {
                headers: { Authorization: `Bearer ${token}` }
            });
            console.log(`✅ Создан район: ${district.name}`);
        } catch (error) {
            console.error(`❌ Ошибка создания района ${district.name}:`, error.response?.data?.message);
        }
    }
}
...
async function createPropertyTypes(token) {
    ...
    for (const type of types) {
        try {
            const response = await axios.post(`${API_BASE}/property-type`, type, {
                headers: { Authorization: `Bearer ${token}` }
            });
            typeIds[type.name] = response.data.id;
            console.log(`✅ Создан тип: ${type.name} (ID: ${response.data.id})`);
        } catch (error) {
            console.error(`❌ Ошибка создания типа ${type.name}:`, error.response?.data?.message);
        }
    }
...
async function createSellers() {
    ...
    for (const seller of sellers) {
        try {
            ...
            sellerTokens[seller.email] = response.data.token;
            console.log(`✅ Создан продавец: ${seller.email}`);
        } catch (error) {
            console.error(`❌ Ошибка создания продавца ${seller.email}:`, error.response?.data?.message);
        }
    }
...
async function createProperties(sellerTokens, typeIds, cityIds) {
    ...
    for (let i = 0; i < baseProperties.length; i++) {
        ...
        try {
            const response = await axios.post(`${API_BASE}/property`, property, {
                headers: { Authorization: `Bearer ${sellerToken}` }
            });
            propertyCount++;
            console.log(`✅ Создана недвижимость: "${property.title}" (${sellerEmail})`);
        } catch (error) {
            console.error(`❌ Ошибка создания недвижимости "${property.title}":`, error.response?.data?.message);
        }
    }
